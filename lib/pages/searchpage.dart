          ...
          appBar: AppBar(
            title: TextFormField(
                    ...
                    onFieldSubmitted: (String s){
                      query = s;
                      print(query);
                      searchLabels = []; // right now, only allowing either filter or search at a time
      ...
      if (prList == null){
        for (int i = 0; i < issueList.length; i++) {
          if (issueList[i].runtimeType == Issue){
            ...
            if (query != '' && issue.title.toLowerCase().contains(query.toLowerCase()) || issue.number == int.tryParse(query)){
        ...
        for (int i = 0; i < prList.length; i++) {
          if (prList[i].runtimeType == PullRequest){
            ...
            if (query != '' && pr.title.toLowerCase().contains(query.toLowerCase()) || pr.number == int.tryParse(query)){
